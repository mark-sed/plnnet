...
:- module(input2, [read_lines/1, split_lines/2, cislo/2, read_lines2/2, split_lines2/2, retezec/2]).
...
cislo([H|T],F,X) :- atom_number(H, H1), FT is 10*F+H1, cislo(T,FT,X).
cislo([],F,F,_).
cislo([H|T],F,X,P) :- atom_number(H, H1), FT is F+H1/P, PT is P*10, cislo(T,FT,X,PT).
